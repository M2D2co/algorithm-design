...
function roll2d6(logging) {
    if (logging === void 0) { logging = true; }
    var result = [randInt(1, 6), randInt(1, 6)];
...
function stats2d6() {
    ...
    var timeBruteForce = end - start;
    var opsBruteForce = t;
    console.table(statsBruteForce);
    // Build Results (Count)
    var statsCount = new Array(12 + 1);
    t = 0;
    start = performance.now();
    for (var i = 2; i <= 12; i++) {
        t++;
...
function rollManyDice() {
    var qtyList = [10, 100, 1000, 10000, 100000, 1000000];
    qtyList.forEach(function (qty) {
        ...
        var stats = results.reduce(function (acc, n) {
            if (!acc[n]) {
                acc[n] = { value: n, count: 1 };
                t++;
            }
